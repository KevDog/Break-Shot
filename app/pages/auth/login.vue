<template>
  <main class="min-h-screen flex items-center justify-center p-4 sm:p-6">
    <div class="w-full max-w-sm bg-bg-secondary rounded-2xl p-6 sm:p-8 ring-1 ring-white/10">
      <h1 class="text-2xl font-semibold text-center mb-1">{{ $t('app.name') }}</h1>
      <p class="text-sm/6 text-text-secondary text-center mb-8">{{ $t('auth.signInToContinue') }}</p>
      ...
      <div class="flex flex-col items-center gap-2 mt-6 text-sm">
        <NuxtLink to="/auth/signup" class="text-text-secondary hover:text-accent no-underline">
          {{ $t('auth.noAccount') }}
        </NuxtLink>
        <NuxtLink to="/auth/forgot-password" class="text-text-secondary hover:text-accent no-underline">
...
useHead({
  title: t('auth.signIn') + ' - Break Shot',
})
</script>
